...
class Swin_JSCC(nn.Module):
    ...
    def gen_embedding(self, bw, snr=None, device=None):
        if snr is None:
            snr = self.link_qual + self.link_rng * np.random.randn(1)[0]
        adapt_embed = torch.from_numpy(np.array([snr, bw])).float().to(device)
        return self.linear_embed(adapt_embed.unsqueeze(0)), snr
    ...
    def forward(self, img, bw, snr=None):
        ...
        # The attention items
        adapt_embed, snr = self.gen_embedding(bw, snr, device=device)

        # Source node
        x = self.enc(img, adapt_embed)
        B, H_W_, n_feat = x.shape  # (B, n_patches, n_trans_feat)
        self.mask = torch.zeros(B, H_W_, self.max_trans_feat * self.unit_trans_feat, device=device)
        ...
        # S->D
        noise_sd = self.complex_sig(noise_shape, device=device)
