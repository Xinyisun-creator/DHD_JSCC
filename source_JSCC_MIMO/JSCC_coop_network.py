...
from .JSCC_utils import complex_sig, pwr_normalize
...
class Div_model(nn.Module):
    def __init__(self,  args, enc, dec):
        ...
        self.c_feat = args.JSCC_cfeat
        self.c_out = args.JSCC_cout

        self.P1 = args.JSCC_P1
        self.P2 = args.JSCC_P2
        self.P1_rng = args.JSCC_P1_rng
        self.P2_rng = args.JSCC_P2_rng

        self.Nr = args.JSCC_Nr
        self.args = args
    ...
    def forward(self, img, is_train):
        ...
        # channel snr settings
        if self.args.JSCC_adapt and is_train:
            P1 = self.P1 + self.P1_rng*(2*torch.rand(1)-1).to(device)
            P2 = self.P2 + self.P2_rng*(2*torch.rand(1)-1).to(device)
